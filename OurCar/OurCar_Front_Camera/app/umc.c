...
/*!
 *  @brief      发送图像到山外多功能上位机显示
 *  @param      imgaddr         图像地址
 *  @param      imgsize         图像占用空间大小
 *  @since      v5.0
 *  @note       不同的上位机，不同的命令，这里使用山外多功能上位机软件，
                如果使用其他上位机，则需要修改代码。
...
void sendimg(uint8_t *imgaddr,uint16_t len)
{
 #define CMD_IMG     1
    ...
    UART_Send_DATA(HW_UART0, cmdf, 2);      //发送帧头命令
    UART_Send_DATA(HW_UART0, imgaddr, len); //再发送图像
    UART_Send_DATA(HW_UART0, cmdr, 2);      //发送帧尾命令
}

//贷默上位机
void DEMOK_sendimg(uint8_t *imgaddr,uint16_t len)
{
	  UART_WriteByte(HW_UART0,0xFF);//发送图像头
    UART_Send_DATA(HW_UART0, imgaddr, len); //再发送图像
}
