...
//行中断
#define OV7725_HREF_PORT  HW_GPIOA
#define OV7725_HREF_PIN   28
//场中断
#define OV7725_VSYNC_PORT HW_GPIOA
#define OV7725_VSYNC_PIN  29
//像素中断
#define OV7725_PCLK_PORT  HW_GPIOA
#define OV7725_PCLK_PIN   27
...
#define  DMA_IRQ_EN(DMA_CHn)    NVIC_EnableIRQ((IRQn_Type)(DMA0_IRQn + DMA_CHn))               //允许DMA通道传输完成中断
#define  DMA_IRQ_DIS(DMA_CHn)   NVIC_DisableIRQ((IRQn_Type)(DMA0_IRQn + DMA_CHn))              //禁止DMA通道传输完成中断



#define  DMA_IRQ_CLEAN(DMA_CHn) DMA0->CINT = DMA_CINT_CINT(DMA_CHn)           //清除通道传输中断标志位

#define  DMA_EN(DMA_CHn)        DMA0->ERQ |= (1<<(DMA_CHn))       //使能通道硬件DMA请求
#define  DMA_DIS(DMA_CHn)       DMA0->ERQ &= ~(1<<(DMA_CHn))     //禁止通道硬件DMA请求


#define  GPIO_IRQ_EN(GPIO_PORT)    NVIC_EnableIRQ((IRQn_Type)(PORTA_IRQn + GPIO_PORT))               //允许DMA通道传输完成中断
#define  GPIO_IRQ_DIS(GPIO_PORT)   NVIC_DisableIRQ((IRQn_Type)(PORTA_IRQn + GPIO_PORT))              //禁止DMA通道传输完成中断

#define  GPIOA_IRQ_CLEAN(GPIO_PIN) PORT_InstanceTable[HW_GPIOA]->ISFR |= 1<<GPIO_PIN         //清除通道传输中断标志位

#define GPIOPORT_IRQ_CLEAN(GPIO_PORT) PORT_InstanceTable[GPIO_PORT]->ISFR = 0xFFFFFFFF
...
void eagle_get_img(void);
void Eagle_ISR(uint32_t index);
void Eagle_Init(uint8_t *imgaddr);
void DMA_ISR(void);
