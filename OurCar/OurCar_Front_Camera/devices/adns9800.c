#include "common.h"
#include "adns9800.h"
#include "spi.h"
#include "gpio.h"
#include "umc.h"
#include "uart.h"


//延时时间，*代表不确定
#define ADNS9800_TIMINGS_OP        	    10      //us*
#define ADNS9800_TIMINGS_INRST 			  	10      //us*
#define ADNS9800_TIMINGS_NCS_SCLK 	  	2       //us*
#define ADNS9800_TIMINGS_POWERON 		  	50      //ms
#define ADNS9800_TIMINGS_SRAD 			  	100     //us
#define ADNS9800_TIMINGS_SRWSRR 		  	20      //us
#define ADNS9800_TIMINGS_FRAME_PERIOD   5000    //us*
#define ADNS9800_TIMINGS_SWW 						120     //us
#define ADNS9800_TIMINGS_LOAD 					15      //us
#define ADNS9800_TIMINGS_FIRMWEND 			160     //us*

//ADNS9800固件
uint8_t ADNS9800_firmwareArray[3070]  = {
 };



extern uint8_t Frame_Capture_Pic[900];
//SPI初始化以及ADNS9800开机等初始化
void ADNS9800_init(void)
{   
	SPI_InitTypeDef SPI_InitStruct1;
	SPI_InitStruct1.baudrate = 750000;
	SPI_InitStruct1.frameFormat = kSPI_CPOL1_CPHA1;
	SPI_InitStruct1.dataSize = 8;
	SPI_InitStruct1.instance = HW_SPI1;
	SPI_InitStruct1.mode = kSPI_Master;
	SPI_InitStruct1.bitOrder = kSPI_MSB;
	SPI_InitStruct1.ctar = HW_CTAR0;
	/* init pinmux */
	PORT_PinMuxConfig(SPI1_PORT, PCS1, kPinAlt2); 
	PORT_PinMuxConfig(SPI1_PORT, PCS0, kPinAlt2); 
	PORT_PinMuxConfig(SPI1_PORT, SCK, kPinAlt2);
	PORT_PinMuxConfig(SPI1_PORT, SOUT, kPinAlt2);
	PORT_PinMuxConfig(SPI1_PORT, SIN, kPinAlt2);
	/* init moudle */
	SPI_Init(&SPI_InitStruct1);
	DelayMs(10);

}


static uint8_t ADNS9800_spi_send(uint8_t data, SPI_PCS_Type csStatus)
{
    return (uint8_t)SPI_ReadWriteByte(HW_SPI1, HW_CTAR0, (uint8_t)data,0, csStatus);
}


static uint8_t ADNS9800_spi_recv(SPI_PCS_Type csStatus)
{
	return (uint8_t)SPI_ReadWriteByte(HW_SPI1, HW_CTAR0, 0x00,0, csStatus);
}

uint8_t bit_is_set(uint8_t data,uint8_t bit)
{
	return (uint8_t)(data>>bit&0x01);
}

void ADNS9800_boot(void)
{
  uint8_t byte;
  // Wait OP + IN-RST for ADNS GO
  DelayUs(ADNS9800_TIMINGS_OP + ADNS9800_TIMINGS_INRST);
	//------------------------------------------------
	// Perform power up reset on current ADNS
	//------------------------------------------------
  // drive NCS high, then low to reset the SPI port
	ADNS9800_spi_cs(0);
	DelayUs(ADNS9800_TIMINGS_NCS_SCLK);
	ADNS9800_spi_cs(1);
	DelayUs(ADNS9800_TIMINGS_NCS_SCLK);
	// Write 0x5A to POWERUP register
	ADNS9800_spi_cs(0);
	DelayUs(ADNS9800_TIMINGS_NCS_SCLK);
	ADNS9800_spi_send(ADNS9800_SPI_WRITE|ADNS9800_SPIREGISTER_POWERUPRESET,kSPI_PCS_KeepAsserted);
	ADNS9800_spi_send(ADNS9800_POWERUPRESET_POWERON, kSPI_PCS_ReturnInactive);
	DelayMs(ADNS9800_TIMINGS_POWERON);
	// read register from 0x02 to 0x06
	uint8_t reg_it;
	for(reg_it=0x02; reg_it<=0x06; reg_it++)
	{
		ADNS9800_spi_send(reg_it,kSPI_PCS_KeepAsserted);
		DelayUs(ADNS9800_TIMINGS_SRAD);
		ADNS9800_spi_recv(kSPI_PCS_ReturnInactive);
		DelayUs(ADNS9800_TIMINGS_SRWSRR);
	}
	// check SPI by reading the productID and inverse productID
	ADNS9800_spi_send(ADNS9800_SPIREGISTER_PRODUCTID,kSPI_PCS_KeepAsserted);
	DelayUs(ADNS9800_TIMINGS_SRAD);
	byte = ADNS9800_spi_recv(kSPI_PCS_ReturnInactive);
	DelayUs(ADNS9800_TIMINGS_SRWSRR);
	ADNS9800_spi_send(ADNS9800_SPIREGISTER_RevisionID,kSPI_PCS_KeepAsserted);
	DelayUs(ADNS9800_TIMINGS_SRAD);
	uint8_t byte_inv = ADNS9800_spi_recv(kSPI_PCS_ReturnInactive);
	DelayUs(ADNS9800_TIMINGS_SRWSRR);
	//------------------------------------------------
	// Load FIRMWARE on current ADNS
	//-----------------------------------------------
  // UART_printf("\nUploading firmware on ADNS9800 #%d",adns_i);
	ADNS9800_upload_firmware();
	//------------------------------------------------
	// Check firmware on ADNS
	//------------------------------------------------
	//ADNS9800_spi_cs(1);
	DelayUs(ADNS9800_TIMINGS_NCS_SCLK);
	// Read SROMID register
	ADNS9800_spi_send(ADNS9800_SPIREGISTER_SROMID,kSPI_PCS_KeepAsserted);
	DelayUs(ADNS9800_TIMINGS_SRAD);
	byte = ADNS9800_spi_recv(kSPI_PCS_ReturnInactive);
	DelayUs(ADNS9800_TIMINGS_SRWSRR);
	//------------------------------------------------
	// Load ADNS configuration
	//------------------------------------------------
	DelayUs(ADNS9800_TIMINGS_NCS_SCLK);
	// Read register current value
	ADNS9800_spi_send(ADNS9800_SPIREGISTER_LASERCTRL0,kSPI_PCS_KeepAsserted);
	DelayUs(ADNS9800_TIMINGS_SRAD);
	byte = ADNS9800_spi_recv(kSPI_PCS_ReturnInactive);
	DelayUs(ADNS9800_TIMINGS_SRWSRR);
	// UART_printf("\nADNS9800 #%d laser ctrl 0 register %x",adns_i,byte);
	// Enable laser, set FORCE_DISABLE bit (bit-0) to 0
	ADNS9800_spi_send(ADNS9800_SPI_WRITE|ADNS9800_SPIREGISTER_LASERCTRL0,kSPI_PCS_KeepAsserted);
	ADNS9800_spi_send(byte&0xFE,kSPI_PCS_ReturnInactive);
	DelayUs(ADNS9800_TIMINGS_SRWSRR);
	// set the CONFIGURATION1 register
	// 0x01 = 50, minimum
	// 0x44 = 3400, default
	// 0x8e = 7100
	// 0xA4 = 8200, maximum
	ADNS9800_spi_send(ADNS9800_SPI_WRITE|ADNS9800_SPIREGISTER_CONFIGURATION1,kSPI_PCS_KeepAsserted);
	ADNS9800_spi_send(ADNS9800_CONFIGURATION1_RESMAX,kSPI_PCS_ReturnInactive);
	DelayUs(ADNS9800_TIMINGS_SRWSRR);
  // Wait maximum delay (a frame period) in case we've got
  // a command immediatly after this function.
  DelayUs(ADNS9800_TIMINGS_FRAME_PERIOD);
  DelayUs(ADNS9800_TIMINGS_FRAME_PERIOD);
	//Power up finished , read register from 0x02 to 0x06 for set them to 0
	for(reg_it=0x02; reg_it<=0x06; reg_it++)
	{
		ADNS9800_spi_send(reg_it,kSPI_PCS_KeepAsserted);
		DelayUs(ADNS9800_TIMINGS_SRAD);
		ADNS9800_spi_recv(kSPI_PCS_ReturnInactive);
		DelayUs(ADNS9800_TIMINGS_SRWSRR);
	}
}





void ADNS9800_upload_firmware(void)
{
  //ADNS9800_spi_cs(0);
  DelayUs(ADNS9800_TIMINGS_NCS_SCLK);
  // Select the 3K bytes SROM size
  ADNS9800_spi_send(ADNS9800_SPI_WRITE|ADNS9800_SPIREGISTER_CONFIGURATION4,kSPI_PCS_KeepAsserted);
  ADNS9800_spi_send(ADNS9800_CONFIGURATION4_LOAD1,kSPI_PCS_ReturnInactive);
  DelayUs(ADNS9800_TIMINGS_SWW);
  // Write 0x1D to register SROMENABLE to initialize
  ADNS9800_spi_send(ADNS9800_SPI_WRITE|ADNS9800_SPIREGISTER_SROMENABLE,kSPI_PCS_KeepAsserted);
  ADNS9800_spi_send(ADNS9800_SROMENABLE_LOAD1,kSPI_PCS_ReturnInactive);
  // Wait for one frame
  DelayUs(ADNS9800_TIMINGS_FRAME_PERIOD);
  // Write 0x18 to register SROMENABLE again to start SROM downloading
  ADNS9800_spi_send(ADNS9800_SPI_WRITE|ADNS9800_SPIREGISTER_SROMENABLE,kSPI_PCS_KeepAsserted);
  ADNS9800_spi_send(ADNS9800_SROMENABLE_LOAD2,kSPI_PCS_ReturnInactive);
  DelayUs(ADNS9800_TIMINGS_SWW);
  // Initiate PROM download burst mode
  ADNS9800_spi_send(ADNS9800_SPI_WRITE|ADNS9800_SPIREGISTER_SROMLOAD,kSPI_PCS_KeepAsserted);
  DelayUs(ADNS9800_TIMINGS_LOAD);
  // For each firmware byte
  uint16_t data_i;
  for(data_i=0;data_i<3070;data_i++)
  {
    // Read firmware from AVR FLASH, then write it
    ADNS9800_spi_send( ADNS9800_firmwareArray[data_i] ,kSPI_PCS_KeepAsserted);
    DelayUs(ADNS9800_TIMINGS_LOAD);
  }
  // Exit burst mode by pulling CS for FIRMWEND
  // note: tFIRMWEND > tBEXIT
  DelayUs(ADNS9800_TIMINGS_FIRMWEND);
}


void Frame_Capture_Prepare(void)
{
	uint8_t byte;
	ADNS9800_spi_cs(0);
	DelayUs(ADNS9800_TIMINGS_NCS_SCLK);
	ADNS9800_spi_cs(1);
	DelayUs(ADNS9800_TIMINGS_NCS_SCLK);
	// Write 0x5A to POWERUP register
	ADNS9800_spi_cs(0);
	DelayUs(ADNS9800_TIMINGS_NCS_SCLK);
	ADNS9800_spi_send(ADNS9800_SPI_WRITE|ADNS9800_SPIREGISTER_POWERUPRESET,kSPI_PCS_KeepAsserted);
	ADNS9800_spi_send(ADNS9800_POWERUPRESET_POWERON, kSPI_PCS_ReturnInactive);
	DelayMs(ADNS9800_TIMINGS_POWERON);
	//ADNS9800_spi_cs(0);
	DelayUs(ADNS9800_TIMINGS_NCS_SCLK);
	// Read register current value
	ADNS9800_spi_send(ADNS9800_SPIREGISTER_LASERCTRL0,kSPI_PCS_KeepAsserted);
	DelayUs(ADNS9800_TIMINGS_SRAD);
	byte = ADNS9800_spi_recv(kSPI_PCS_ReturnInactive);
	DelayUs(ADNS9800_TIMINGS_SRWSRR);
	// Enable laser, set FORCE_DISABLE bit (bit-0) to 0
	ADNS9800_spi_send(ADNS9800_SPI_WRITE|ADNS9800_SPIREGISTER_LASERCTRL0,kSPI_PCS_KeepAsserted);
	ADNS9800_spi_send(byte&0xFE,kSPI_PCS_ReturnInactive);
	DelayUs(ADNS9800_TIMINGS_SRWSRR);
}

uint8_t Motion=0;
uint8_t Get_Motion_start=0;
uint8_t Frame_Capture_start=0;
void Frame_Capture(uint8_t * pic)
{
	
	uint8_t byte;
	//第一次进入时开机复位，开激光
	Get_Motion_start=0;
	if(Frame_Capture_start<2)
	{
		Frame_Capture_start++;
	}
	if(Frame_Capture_start==1)
	{
		Frame_Capture_Prepare();
	}
	DelayUs(ADNS9800_TIMINGS_NCS_SCLK);
	//Write 0x93 to register ADNS9800_SPIREGISTER_Frame_Capture
  ADNS9800_spi_send(ADNS9800_SPI_WRITE|ADNS9800_SPIREGISTER_Frame_Capture,kSPI_PCS_KeepAsserted);
  ADNS9800_spi_send(ADNS9800_SROMENABLE_Frame_Capture1,kSPI_PCS_ReturnInactive);
  DelayUs(ADNS9800_TIMINGS_SWW);
  //Write 0xC5 to register ADNS9800_SPIREGISTER_Frame_Capture
  ADNS9800_spi_send(ADNS9800_SPI_WRITE|ADNS9800_SPIREGISTER_Frame_Capture,kSPI_PCS_KeepAsserted);
  ADNS9800_spi_send(ADNS9800_SROMENABLE_Frame_Capture2,kSPI_PCS_ReturnInactive);
	//Wait for two frame
	DelayUs(ADNS9800_TIMINGS_FRAME_PERIOD);
	DelayUs(ADNS9800_TIMINGS_FRAME_PERIOD);
	// Read register ADNS9800_SPIREGISTER_MOTION
	ADNS9800_spi_send(ADNS9800_SPIREGISTER_MOTION,kSPI_PCS_KeepAsserted);
	DelayUs(ADNS9800_TIMINGS_SRAD);
	byte = ADNS9800_spi_recv(kSPI_PCS_ReturnInactive);
  Motion = byte;
	DelayUs(ADNS9800_TIMINGS_SRWSRR);
	if(bit_is_set(byte,0)==1)
	{
		// Read  the first register Pixel_Burst
		ADNS9800_spi_send(ADNS9800_SPIREGISTER_Pixel_Burst,kSPI_PCS_KeepAsserted);
		DelayUs(ADNS9800_TIMINGS_SRAD);
		for(int16_t i=0;i<900;i++)
		{
			// Read  Pixel_Burst register current value
			*(pic+i) = ADNS9800_spi_recv(kSPI_PCS_KeepAsserted);
			DelayUs(ADNS9800_TIMINGS_LOAD);
		}
		ADNS9800_spi_cs(1);
	}
}


void Get_Motion(int16_t * Motion_X,int16_t * Motion_Y)
{
		static uint8_t Get_Motion_start=0;
		uint8_t buf[4]={0,0,0,0};
	  uint8_t * bufp=buf;
  	uint8_t reg_it;
	  //第一次进入时开机复位，下固件，开激光
	  Frame_Capture_start=0;
		if(Get_Motion_start<2)
		{
			Get_Motion_start++;
		}
		if(Get_Motion_start==1)
		{
			ADNS9800_boot();
		}else
		{
			//read register Motion
			ADNS9800_spi_send(ADNS9800_SPIREGISTER_MOTION,kSPI_PCS_KeepAsserted);
			DelayUs(ADNS9800_TIMINGS_SRAD);
			ADNS9800_spi_recv(kSPI_PCS_ReturnInactive);
			DelayUs(ADNS9800_TIMINGS_SRWSRR);	
			// read register from 0x03 to 0x06
			for(reg_it=0x03; reg_it<=0x06; reg_it++)
			{
				ADNS9800_spi_send(reg_it,kSPI_PCS_KeepAsserted);
				DelayUs(ADNS9800_TIMINGS_SRAD);
				*(bufp++)=ADNS9800_spi_recv(kSPI_PCS_ReturnInactive);
				DelayUs(ADNS9800_TIMINGS_SRWSRR);
			}
			*Motion_X = -(buf[0]|(uint16_t)buf[1]<<8);
			*Motion_Y =  (buf[2]|(uint16_t)buf[3]<<8);
  	}
}


