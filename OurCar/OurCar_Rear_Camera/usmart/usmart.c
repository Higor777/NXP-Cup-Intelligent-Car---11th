...
void usmart_scan(void)
{
	u8 sta;
	u16 len;  
	if(UART_RX_STA[1]==USMART_UART_DATA)//串口接收完成？
	{					   
		len=UART_RX_STA[0];	//得到此次接收到的数据长度
		UART_RX_DATA[len]='\0';	//在末尾加入结束符. 
		sta=usmart_dev.cmd_rec(UART_RX_DATA);//得到函数各个信息
		if(sta==0)usmart_dev.exe();	//执行函数 
		else 
		{  
			len=usmart_sys_cmd_exe(UART_RX_DATA);
		...
		UART_RX_STA[0]=0;
		UART_RX_STA[1]=NONE_UART_DATA;//状态寄存器清空	    
	}
}
